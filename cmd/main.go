...
import (
	...
	event_stream "github.com/turistikrota/service.booking/server/event-stream"
	"github.com/turistikrota/service.booking/server/http"
	"github.com/turistikrota/service.booking/service"
...
func loadMongo(cnf config.App) *mongo.DB {
	uri := mongo.CalcMongoUri(mongo.UriParams{
		Host:  cnf.DB.Booking.Host,
		Port:  cnf.DB.Booking.Port,
		User:  cnf.DB.Booking.Username,
		Pass:  cnf.DB.Booking.Password,
		Db:    cnf.DB.Booking.Database,
		Query: cnf.DB.Booking.Query,
	})
	d, err := mongo.New(uri, cnf.DB.Booking.Database)
	if err != nil {
		panic(err)
	}
	return d
